# Scoring and score card: proposal
...
29 takes, 25–26 July 2026, covering every song where a captured probe trace
...
song at all". Across the corpus, notes-attempted ran **91–100%**, so that question
...
explicitly** (as a dash, with the reason) rather than quietly folding it in. The two
...
- `pitch_sample_count = sample_rate.div_ceil(40)` is a 25 ms window, and
...
band across all 29 takes, the tracker's own noise floor rather than a property of the
...
- **Times sung is in**, as a detail rather than a metric: "first time on this one",
  "3rd time you've sung this". It comes free by counting occurrences of
...
synced override for the two real exceptions, testing on a packaged build and
...
- **How harsh is too harsh.** A more discriminating formula hands out C and D that
  ...
  presentation, leading with the best axis, naming the concrete failure and
  keeping the ribbon, not arithmetic. Worth a decision before shipping.

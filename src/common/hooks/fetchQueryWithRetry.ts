...
// arrives in milliseconds. The browser's own fetch timeout is ~5 minutes, and
// a request that hangs instead of erroring (a request landing mid server
// boot, a half-open socket) would otherwise stall the retry loop for that
// whole window. Treat a slow attempt as failed and retry.
const ATTEMPT_TIMEOUT_MS = 10000;

// The initial fetch a hook fires on mount can race the GraphQL server coming
// up (dev launches the renderer alongside the server), and the first request
// after a fresh launch can flake (a Parcel lazy-module race aggravated by
// Sentry's require patching; see CLAUDE.md). A plain fetchQuery with no error
// handler fails silently and the setting never syncs, so retry with capped
// backoff until the query succeeds or the hook unmounts.

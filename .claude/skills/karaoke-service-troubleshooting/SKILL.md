...
anything, because conflating them wastes hours.
...
5. **A stale binary is not the only cause, so check for HTTP 429 first.** If the
...
`youtubeSpawnEnv()`, which the runtime inherits), so no extra runtime to ship.

Note the search path (youtubei.js) and the download path (yt-dlp) are
**independent**, so search can work perfectly while downloads are bot-walled.
...
- DAM's `cds1-clubdam...ipcasting.jp` CDN **403s from datacenter/VPN exit
  ...
  and the official DAM Windows client fails on the same network state and
...
- **Cycling the VPN is no longer the fix. NordVPN is dead for this.** Its
  ...
  so score candidates with `dam-exit-check.sh` before committing to one. Set
